...
namespace Volvox.Helios.Core.Modules.Command.Commands
{
    public class AboutCommand : ICommand
    {
        ...
        {
            if (!context.Message.Content.StartsWith($"{context.GivenPrefix}about", StringComparison.InvariantCulture)) throw new TriggerFailException();
            await Execute(context);
        }

        public async Task Execute(CommandContext context)
        {
            if (context.Channel is not SocketGuildChannel channel) return;
